...
class fpn_module_global(nn.Module):
    def __init__(self, numClass):
        ...
        # self.classify_ext = nn.Conv2d(128*4*2, numClass, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, c2, c3, c4, c5, c2_ext=None, c3_ext=None, c4_ext=None, c5_ext=None, ps0_ext=None, ps1_ext=None, ps2_ext=None):
        ...
        if c5_ext is None:
            ...
            # p5 = self.toplayer(c5 + c5_ext)
            # p4 = self._upsample_add(p5, self.latlayer1(c4 + c4_ext))
            # p3 = self._upsample_add(p4, self.latlayer2(c3 + c3_ext))
            # p2 = self._upsample_add(p3, self.latlayer3(c2 + c2_ext))
        ...
        if ps0_ext is None:
            ...
            # p5 = self.smooth1_1(p5 + ps0_ext[0])
            # p4 = self.smooth2_1(p4 + ps0_ext[1])
            # p3 = self.smooth3_1(p3 + ps0_ext[2])
            # p2 = self.smooth4_1(p2 + ps0_ext[3])
        ...
        if ps1_ext is None:
            ...
            # p5 = self.smooth1_2(p5 + ps1_ext[0])
            # p4 = self.smooth2_2(p4 + ps1_ext[1])
            # p3 = self.smooth3_2(p3 + ps1_ext[2])
            # p2 = self.smooth4_2(p2 + ps1_ext[3])
...
class fpn_module_local(nn.Module):
    ...
    def forward(self, c2, c3, c4, c5, c2_ext=[], c3_ext=[], c4_ext=[], c5_ext=[], ps0_ext=[[], [], [], []], ps1_ext=[[], [], [], []], ps2_ext=[[], [], [], []]):
        ...
        # Top-down
        p5 = self.toplayer(torch.cat([c5] + [F.upsample(c5_ext[0], size=c5.size()[2:], mode='bilinear')], dim=1))
        p4 = self._upsample_add(p5, self.latlayer1(torch.cat([c4] + [F.upsample(c4_ext[0], size=c4.size()[2:], mode='bilinear')], dim=1)))
        p3 = self._upsample_add(p4, self.latlayer2(torch.cat([c3] + [F.upsample(c3_ext[0], size=c3.size()[2:], mode='bilinear')], dim=1)))
        p2 = self._upsample_add(p3, self.latlayer3(torch.cat([c2] + [F.upsample(c2_ext[0], size=c2.size()[2:], mode='bilinear')], dim=1)))
        ps0 = [p5, p4, p3, p2]
        
        # Smooth
        p5 = self.smooth1_1(torch.cat([p5] + [F.upsample(ps0_ext[0][0], size=p5.size()[2:], mode='bilinear')], dim=1))
        p4 = self.smooth2_1(torch.cat([p4] + [F.upsample(ps0_ext[1][0], size=p4.size()[2:], mode='bilinear')], dim=1))
        p3 = self.smooth3_1(torch.cat([p3] + [F.upsample(ps0_ext[2][0], size=p3.size()[2:], mode='bilinear')], dim=1))
        p2 = self.smooth4_1(torch.cat([p2] + [F.upsample(ps0_ext[3][0], size=p2.size()[2:], mode='bilinear')], dim=1))
        ps1 = [p5, p4, p3, p2]
        
        p5 = self.smooth1_2(torch.cat([p5] + [F.upsample(ps1_ext[0][0], size=p5.size()[2:], mode='bilinear')], dim=1))
        p4 = self.smooth2_2(torch.cat([p4] + [F.upsample(ps1_ext[1][0], size=p4.size()[2:], mode='bilinear')], dim=1))
        p3 = self.smooth3_2(torch.cat([p3] + [F.upsample(ps1_ext[2][0], size=p3.size()[2:], mode='bilinear')], dim=1))
        p2 = self.smooth4_2(torch.cat([p2] + [F.upsample(ps1_ext[3][0], size=p2.size()[2:], mode='bilinear')], dim=1))
        ps2 = [p5, p4, p3, p2]

        # Classify
        # use ps2_ext
        ps3 = self._concatenate(
                torch.cat([p5] + [F.upsample(ps2_ext[0][0], size=p5.size()[2:], mode='bilinear')], dim=1), 
                torch.cat([p4] + [F.upsample(ps2_ext[1][0], size=p4.size()[2:], mode='bilinear')], dim=1), 
                torch.cat([p3] + [F.upsample(ps2_ext[2][0], size=p3.size()[2:], mode='bilinear')], dim=1), 
                torch.cat([p2] + [F.upsample(ps2_ext[3][0], size=p2.size()[2:], mode='bilinear')], dim=1)
            )
        ps3 = self.smooth(ps3)
        output = self.classify(ps3)
...
class fpn(nn.Module):
    def __init__(self, numClass):
        ...
        #         nn.Conv2d(128*4 * 2, 128*4, kernel_size=3, stride=1, padding=1, dilation=1),
        #         nn.Conv2d(128*4, numClass, kernel_size=3, stride=1, padding=1, dilation=1)
        #         )
    ...
    def _crop_global(self, f_global, top_lefts, ratio):
        ...
        # crop_1x = self._sample_grid(f_global, bbox, (H, W))
        # bbox = [ np.array([left/(1+ratio), top/(1+ratio), left/(1+ratio) + 2*ratio, top/(1+ratio) + 2*ratio]) for (top, left) in top_lefts ]
        # crop_2x = self._sample_grid(F.pad(f_global, (w//2, w//2, h//2, h//2), mode='reflect'), bbox, (H, W))
        ...
        for i in range(b):
            ...
            crop_1x.append(f_global_patch[0])
        #     
        #     h_2x = int(min(top - 0.5*h, 0)) + 2*h; w_2x = int(min(left - 0.5*w, 0)) + 2*w;
        #     top_2x = int(max(top - 0.5*h, 0)); left_2x = int(max(left - 0.5*w, 0));
        #     f_global_patch_2x = F.upsample(f_global[0:1, :, top_2x:top_2x+h_2x, left_2x:left_2x+w_2x], size=(H, W), mode='bilinear')
        #     crop_2x.append(f_global_patch_2x[0])
        crop_1x = torch.stack(crop_1x, dim=0) # stack into mini-batch
        # crop_2x = torch.stack(crop_2x, dim=0) # stack into mini-batch
        return [crop_1x]#, crop_2x] # b, c, H, W
    ...
    def _merge_local(self, f_local, merge, f_global, top_lefts, oped, ratio, template):
        ...
        if oped[1] >= len(top_lefts):
            ...
            merge /= template
            # avg_pool AND upsample
            # merge = F.avg_pool2d(merge, 2, stride=2, padding=0)
            # merge = F.upsample(merge, size=(H, W), mode='bilinear')
            ##########################
        return merge
    ...
    def ensemble(self, f_local, f_global):
        ...
        # weights = F.sigmoid(self.ensemble_conv(torch.cat((f_local, f_global), dim=1)))
        # return f_local + weights * f_global
    ...
    def forward(self, image_global, patches, top_lefts, ratio, mode=1, global_model=None, n_patch=None):
        if mode == 1:
            ...
            ps3_g2l = F.upsample(ps3_g2l, size=ps3_l.size()[2:], mode='bilinear')
